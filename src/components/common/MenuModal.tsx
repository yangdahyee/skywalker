...
export default function MenuModal({ onClose }: MenuModalProps) {
  ...
  useEffect(() => {
    // 팝업 창이 열릴 때 GSAP 애니메이션 가동 (scale과 opacity를 정상화)
    if (modalRef.current) {
  ...
  const handleClose = () => {
    if (modalRef.current) {
      gsap.to(modalRef.current, {
        ...
        duration: 0.25,
        ease: "power2.in",
        onComplete: onClose, // 애니메이션이 완전히 끝난 후 상위 컴포넌트의 상태를 false로 변경
      })
    } else {
      onClose()
    }
  ...
  // 🧭 단일 진실 공급원 주소 성계 워프 내비게이션 핸들러
  const handleNavigate = (path: string) => {
    navigate(path)
    onClose() // 이동 완료 후 폴더 레이어 닫기
  }
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/40 z-99999 backdrop-blur-sm flex items-center justify-center font-mono cursor-default" style={{ cursor: "default" }}>
      <div
        ref={modalRef}
        className="bg-[#FFFDF9] border-[4px] border-slate-900 rounded-2xl p-1 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] w-[460px] flex flex-col overflow-hidden select-none text-slate-900"
      >
        {/* 상단 윈도우 컨트롤 헤더 */}
